...
When the client makes a `POST` request to `/api/users`: DONE 
...
When the client makes a `GET` request to `/api/users/:id`: DONE
...
When the client makes a `DELETE` request to `/api/users/:id`: DONE 
